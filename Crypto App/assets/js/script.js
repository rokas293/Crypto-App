...
function openTab(event, tabName) {
    ...
    if (!tabDataLoaded[tabName]) {
        switch (tabName){
            case 'tab1':
                fetchAndDisplay('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=20&page=1&sparkline=true', ['asset-list'], displayAssets, tabName, 'Crypto Data');
                break;
            case 'tab2':
...
async function fetchData(){
    await Promise.all([
        ...
        fetchAndDisplay('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=20&page=1&sparkline=true', ['asset-list'], displayAssets, null, 'Crypto Data'),
    ]);
}
...
function displayExchanges(data) {
    ...
    const table = createTable(['Rank', 'Exchange', 'Trust Score', '24h Trage', '24h Trade(Normal)', 'Country', 'Website', 'Year'], 1);
